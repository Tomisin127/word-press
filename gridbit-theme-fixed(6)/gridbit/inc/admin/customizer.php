...
function gridbit_register_theme_customizer( $wp_customize ) {

    if(method_exists('WP_Customize_Manager', 'add_panel')):
    $wp_customize->add_panel('gridbit_main_options_panel', array( 'title' => esc_html__('Theme Options', 'gridbit'), 'priority' => 10, ));
    endif;

    $wp_customize->get_section( 'title_tagline' )->panel = 'gridbit_main_options_panel';
    $wp_customize->get_section( 'title_tagline' )->priority = 20;
    $wp_customize->get_section( 'header_image' )->panel = 'gridbit_main_options_panel';
    $wp_customize->get_section( 'header_image' )->priority = 26;
    $wp_customize->get_section( 'background_image' )->panel = 'gridbit_main_options_panel';
    $wp_customize->get_section( 'background_image' )->priority = 27;
    $wp_customize->get_section( 'colors' )->panel = 'gridbit_main_options_panel';
    ...
    $wp_customize->get_control( 'background_color' )->description = esc_html__('To change Background Color, need to remove background image first:- go to Appearance : Customize : Theme Options : Background Image', 'gridbit');

    if ( isset( $wp_customize->selective_refresh ) ) {
        $wp_customize->selective_refresh->add_partial( 'blogname', array(
            'selector'        => '.gridbit-site-title a',
            'render_callback' => 'gridbit_customize_partial_blogname',
        ) );
        $wp_customize->selective_refresh->add_partial( 'blogdescription', array(
            'selector'        => '.gridbit-site-description',
            'render_callback' => 'gridbit_customize_partial_blogdescription',
        ) );
    }

    gridbit_getting_started($wp_customize);
    gridbit_menu_options($wp_customize);
    gridbit_header_options($wp_customize);
    gridbit_posts_grid_options($wp_customize);
    gridbit_post_options($wp_customize);
    gridbit_navigation_options($wp_customize);
    gridbit_page_options($wp_customize);
    gridbit_social_profiles($wp_customize);
    gridbit_footer_options($wp_customize);
    gridbit_search_404_options($wp_customize);
    gridbit_other_options($wp_customize);

}

add_action( 'customize_register', 'gridbit_register_theme_customizer' );
 ...
 * @return void
 */
function gridbit_customize_partial_blogname() {
    bloginfo( 'name' );
}
/**
 ...
 * @return void
 */
function gridbit_customize_partial_blogdescription() {
    bloginfo( 'description' );
}

function gridbit_customizer_js_scripts() {
    wp_enqueue_script('gridbit-theme-customizer-js', get_template_directory_uri() . '/inc/admin/js/customizer.js', array( 'jquery', 'customize-preview' ), null, true);
}
add_action( 'customize_preview_init', 'gridbit_customizer_js_scripts' );